// api/proxy.js
// CORS Anywhere gaya asli via Vercel Functions (handler single-file)

const corsAnywhere = require('cors-anywhere');

// Ambil whitelist dari ENV (pisahkan dengan koma), contoh:
...
// Ikuti perilaku default cors-anywhere asli:
...
module.exports = (req, res) => {
  // /api/proxy dipanggil tanpa target → bantu dengan demo
  if (req.url === '' || req.url === '/' || req.url === '/proxy' || req.url === '/api/proxy') {
    return demoPage(res);
  }

  // /corsdemo (direwrite di vercel.json)
  ...
  // 4) Path  style: .../https://target           (direwrite ke sini oleh vercel.json)
